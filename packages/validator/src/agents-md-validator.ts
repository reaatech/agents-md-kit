...
import type { AgentsMdDocument, Finding, ValidationResult } from '@reaatech/agents-markdown';
import { AgentsMdFrontmatterSchema } from '@reaatech/agents-markdown';
...
export function validateAgentsMd(
  ...
  const skillSection = findSectionByTitle(document.sections, 'Skill System');
  if (skillSection) {
  ...
  const observabilitySection = findSectionByTitle(document.sections, 'Observability');
