import type { Section } from '@reaatech/agents-markdown';
...
`;
    const document = await parseMarkdown(content, 'AGENTS.md');
    const headings = document.sections.flatMap((s: Section) => [s, ...s.subsections]);
    const subSub = headings.find((h: Section) => h.title === 'SubSub heading');
    if (subSub) {
      const result = validateAgentsMd(document);
...
`;
    const document = await parseMarkdown(content, 'AGENTS.md');
    const { runLintRules } = await import('@reaatech/agents-markdown-linter');
