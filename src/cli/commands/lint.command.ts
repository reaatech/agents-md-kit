...
export function lintCommand(program: Command): void {
  program
    ...
    .option('-s, --severity <level>', 'Minimum severity to report (error, warning, info, suggestion)', 'info')
    ...
    .action(async (targetPath: string, options: {
      ...
      await recordOperation('lint', async () => {
        ...
        const filteredResults = results.map((result) => filterLintResult(result, options.severity));
